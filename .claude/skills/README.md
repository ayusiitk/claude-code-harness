...
|---|---|---|
| `using-skills` | superpowers `using-superpowers`, mattpocock `ask-matt` | Rewritten around size and risk classification. Added context-inspection-first, the terse classification line and the no-process-recursion rule. The abstract consequential-decisions stop rule was replaced by a contract gate sitting between classification and execution: classification chooses the workflow, it does not authorize implementation. Decomposed after a control experiment showed the gate fires in isolation and not inside a large composite skill, so the spine carries the decisions and `references/` carries the detail. Dropped the multi-harness platform section. |
